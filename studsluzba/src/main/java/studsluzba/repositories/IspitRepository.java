...
public interface IspitRepository extends CrudRepository<Ispit, Integer> {
	...
	@Query( value = "select s.idstudent, (spo.poeni + i.brPoena) from Student s"
			+ " join StudIndex si on (s.idstudent = si.idIndex)"
			+ " join IzlazakNaIspit i on (i.idStudIndex = si.idIndex)"
			...
			+ " order by si.idstudProgram, si.godinaUpisa, si.broj", nativeQuery = true)
	...
	@Query("select spo.poeni from StudentPredispitneObaveze spo"
			...
			+ " join Student s on (dp.student.idstudent = s.idstudent)"
			+ " where dp.sg.idSkolskeGodine = :idSkolskeGodine and s.idstudent = :idStudenta and dp.predmet.idPredmeta = :idPredmeta")
	Double findPoeniStudentPredmet(int idStudenta, int idPredmeta, int idSkolskeGodine);
